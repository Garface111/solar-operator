"""
READ-ONLY diagnostic (prod-safe): why our GMCS monthly generation numbers
diverge from the values Crown REC Services submits to NEPOOL-GIS.

This is the branch script (scripts/diag_crown_parity.py) adapted to run against
the CURRENTLY-DEPLOYED writer on main, which does NOT yet have the
`default_reporting_reference_date` lag-fix symbol. Instead of importing that, we
derive the evaluation window directly from Crown's ground-truth months
(2024-07 .. 2026-03) so we score every month we have truth for. Sourcing logic
(daily-wins-else-bill) is reproduced EXACTLY with the two functions that DO exist
on prod: `_daily_generation_by_month` (DailyGeneration) and
`distribute_kwh_by_calendar_day` (bill fallback). Writes nothing.

  railway ssh "cat > /app/diag_crown_parity.py && cd /app && \
      PYTHONPATH=/app python diag_crown_parity.py --tenant ten_14b76982523a3b47; \
      rm -f /app/diag_crown_parity.py" < diag_crown_parity_prod.py
"""
...
from api.writers.gmcs_writer import _daily_generation_by_month
...
def _crown_window():
    """Evaluation window = full span of Crown's ground-truth months."""
    keys = set()
    for v in CROWN_TRUTH.values():
        keys.update(v["monthly_mwh"].keys())
    ym = sorted((int(k[:4]), int(k[5:7])) for k in keys)
    months = []
    y, m = ym[0]
    end_y, end_m = ym[-1]
    while (y, m) <= (end_y, end_m):
        ...
        if m == 13:
            y, m = y + 1, 1
    report_start = date(ym[0][0], ym[0][1], 1)
    ey, em = ym[-1]
    report_end = (date(ey, 12, 31) if em == 12
                  else date(ey, em + 1, 1) - timedelta(days=1))
    return months, report_start, report_end
...
def main() -> None:
    ...
    months, report_start, report_end = _crown_window()
    print(f"tenant={args.tenant}  window={months[0]}..{months[-1]}  "
          f"({report_start}..{report_end})  [derived from Crown ground-truth span]")
    print("(NOTE: prod writer default window is date.today()-relative; this diag "
          "scores every month Crown has truth for, which is what parity needs)\n")
