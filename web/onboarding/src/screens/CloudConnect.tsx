...
// Curated quick-picks — VT/NH NEPOOL majority. host = SmartHub subdomain
// (null for GMP, which uses its own login).
interface QuickPick {
  code: string;
  name: string;
  host: string | null;
...
export default function CloudConnect() {
    ...
    try {
      ...
      if (res.session_token) {
        ...
        try {
          ...
          /* private mode — session still in memory */
        }
        // Persist cloud mode so the dashboard opens the Cloud Capture vault.
        void setCaptureModeCloud(res.session_token).catch(() => {});
      } else {
        setCompleteError(
          "Your account is set up, but we couldn't start a secure session to store logins. You can add them from Master account after signing in.",
        );
      }
    } catch (e) {
      setCompleteError(
  ...
  return (
    <ScreenLayout current={3} stepLabelsOverride={["Welcome", "Your info", "Clients", "Your logins", "Done"]}>
      <Card active>
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">
          Connect a utility — we&apos;ll do the rest.
        </h1>
        <p className="mt-2 text-sm text-zinc-500">
          Add the login you use for a client&apos;s utility portal. Our servers sign in and
          pull the bills for you — encrypted at rest, never shown again, remove anytime.
        </p>
          ...
          {!selected ? (
            <>
              <p className="text-sm font-semibold text-zinc-900">
                {saved.length > 0 ? "Add another utility" : "Which utility do your clients use?"}
              </p>
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
              ...
              <p className="mt-2 text-xs text-zinc-500">
                Stored encrypted on our servers so we can refresh bills automatically.
                Remove anytime from Master account.
              </p>
        ...
        <div className="mt-8 flex items-center justify-between gap-4">
          <span className="text-xs text-zinc-500">
            {saved.length > 0
              ? "You can add more or manage logins later from Master account."
              : "You can also add logins later from Master account."}
          </span>
          <Button
