...
export default function Done() {
  ...
  useEffect(() => {
    ...
    async function finish() {
      // Cloud fork (/cloud) completes onboarding EARLY so it can store logins
      // with a live session. If it already did, don't complete a second time —
      // that would fire a duplicate magic-link/sample email.
      let alreadyCompleted = false;
      try {
        alreadyCompleted = sessionStorage.getItem(ONBOARDING_COMPLETED_KEY) === "1";
      } catch {
        /* sessionStorage locked down — fall through to normal complete */
      }
      if (alreadyCompleted) {
        try {
