...
import { cloudCaptureUiEnabled } from "../lib/onboarding";

/**
 * Connect fork — the operator chooses HOW their utility bills get pulled:
 *   • Store it with us (Cloud Capture) → /cloud (server-side, zero effort)
 *   • Keep it on my computer (extension) → /extension (the unchanged flow)
 *
 * DARK-SHIPPED: only reachable when `so:flag:cloud-capture-ui` is on. If it isn't
 * (a real signup), we redirect straight to the extension flow so nothing changes.
 */
    ...
    <ScreenLayout current={3}>
      <Card active>
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">
          How should we keep your bills fresh?
        </h1>
        <p className="mt-2 text-sm text-zinc-500">
          Two ways to connect your utility. Pick whichever fits — you can switch later
          from your account settings.
        </p>

        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {/* Cloud — recommended, zero-effort */}
          <button
            type="button"
          ...
          >
            ...
            <p className="mt-1 flex-1 text-sm text-zinc-600">
              Enter your utility login once and our servers sign in and pull the bills for
              you around the clock — no install, no tab to keep open. Your password is
          ...
          >
            ...
            <p className="mt-1 flex-1 text-sm text-zinc-600">
              Install the free browser extension. It captures your bills while you&apos;re
