...
def test_placeholder_adopted_on_first_capture(client):
    ...
    with SessionLocal() as db:
        ...
        # Renamed using the portal-side account-holder name (fullName from
        # the payload). Per the smart-name split (Bruce June 5), holder
        # name beats email. The payload here ships "Captured User" as the
        # holder — that becomes the client name.
        assert c.name == "Captured User", f"expected holder-name, got name={c.name!r}"
