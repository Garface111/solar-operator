"""VIP watch — the tight (minutes, not days) self-heal staleness bar AND the
Ford-alert sweep both apply to EVERY non-demo tenant, with ONE bar, no tiers
(Ford, 2026-07-08: "all accounts should be babied like this" ... then, after
finding the alert half had been secretly tiered to protect his inbox: "we are
hardcore not babies who try to conserve electricity... find every instance of
us intentionally sabotaging our own reliability").
...
recovery, for every ACTIVE, NON-DEMO tenant, all on the SAME ALERT_AFTER_MINUTES
bar -- no tenant gets a slower/quieter tier; (3) is_demo=True tenants are
...
def test_sweep_alerts_once_then_dedups_then_clears_on_recovery(monkeypatch):
    # vip_watch_sweep scans EVERY active non-demo tenant, and this suite's
    # other tests leave their own tenants sitting in the SAME database (no
    # per-test rollback). Scope every assertion to THIS test's tenant so the
    ...
    tid = _mk_tenant()
    now_ = datetime.utcnow()
    stale_at = now_ - timedelta(minutes=ALERT_AFTER_MINUTES + 10)
...
def test_sweep_excludes_demo_tenants(monkeypatch):
    """is_demo=True tenants never alert, no matter how stale -- the real fix
    for the inbox-noise problem (not a slower bar for real customers)."""
    sent = []
    monkeypatch.setattr("api.notify.send_internal_alert",
                        lambda subject, body: sent.append((subject, body)) or True)
    monkeypatch.setattr("api.inverter_fleet._daylight_for", lambda arr, default, _cache=None: True)
    tid = _mk_tenant(is_demo=True)
    now_ = datetime.utcnow()
    _mk_array_with_inverter(tid, "sma", now_ - timedelta(minutes=ALERT_AFTER_MINUTES + 1000))
    out = vip_watch_sweep()
    assert [s for s in sent if tid in s[1]] == []
    assert [a for a in out["alerted"] if a["tenant_id"] == tid] == []


def test_sweep_uses_one_universal_bar_no_tiers(monkeypatch):
    """Any non-demo tenant alerts past ALERT_AFTER_MINUTES -- there is no
    slower/quieter tier for anyone."""
    sent = []
    monkeypatch.setattr("api.notify.send_internal_alert",
                        lambda subject, body: sent.append((subject, body)) or True)
    monkeypatch.setattr("api.inverter_fleet._daylight_for", lambda arr, default, _cache=None: True)
    tid = _mk_tenant()
    now_ = datetime.utcnow()
    array_id = _mk_array_with_inverter(tid, "sma", now_ - timedelta(minutes=ALERT_AFTER_MINUTES + 5))
    out = vip_watch_sweep()
