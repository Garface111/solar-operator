...
_MORNING_NOTE_SYSTEM = (
    ...
    "utility, then state the fix using ONLY that problem's own 'detail' and 'fix' "
    "fields, in the owner's own terms. NEVER invent an action for them — in "
    "particular, do not tell anyone to re-connect, re-authorize or log into a utility "
    "portal unless that problem's 'fix' says so. A problem carrying "
    "capture_mode='cloud' means we hold the password and sign in on our side, so "
    "there is nothing for them to re-connect; say what its 'fix' says instead.\n"
