...
Before a live import, establish a verified safe onboarding state. The existing pause stops scheduled delivery/drafting, but manual actions and another review job are not universally stopped. Use a staging database, email recipient sinks and payment sandbox credentials for rehearsal. Preview must write nothing. Commit must be atomic, or any partial-import workflow must be explicit and reviewed. Created, duplicate, rejected and unresolved counts must account for every source row.

## Engineering exit criteria

1. Every independent launch acceptance test passes without waiver, skip or xfail. Make the separate launch gate a mandatory release check.
...
8. Restore a staging backup and demonstrate recovery of invoice/payment history and pending/uncertain work. Establish production backup retention and recovery ownership. No restore drill was performed in this audit.
...
- **Rejected roster batch:** identify rows that actually committed before retry. Current production behavior can leave a partial roster; a failed row does not imply zero changes.
...
- **Provider throttling/outage:** persist retry state, honor backoff and alert after a defined retry budget. Keep successful and uncertain sends out of blind retries.
