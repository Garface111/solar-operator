# Sovereign docs

Plans, research notes, and **standing survival policy** for the Sovereign Mind.

## Read first (survival)

| Doc | Why |
|-----|-----|
| **[HOW_NOT_TO_CRASH.md](./HOW_NOT_TO_CRASH.md)** | Failure conditions A–H and hard laws so Sovereign does not take Array Operator down again |

## Related plans (in `docs/plans/`)

| Doc | Topic |
|-----|--------|
| `2026-07-15-energy-agent-sovereign-mind.md` | Mind architecture, layers, authority |
| `2026-07-16-process-split-web-worker.md` | Web vs worker process split |
| `2026-07-16-sovereign-reenable-after-split.md` | Safe flag re-enable ladder + guard |
| `2026-07-16-sovereign-durability-watchdog.md` | Dual-channel watchdog / storm breaker |
| `2026-07-16-sovereign-skill-evolution.md` | Hermes-style skill evolution |

## Reality file + Mind sandbox + Rocket engine (2026-07-22)

| Doc | Why |
|-----|-----|
| **[ROCKET_ENGINE.md](./ROCKET_ENGINE.md)** | **The vision** — locked room, oxidizer, nozzle, score (read this first) |
| **[CLOUD_RUN.md](./CLOUD_RUN.md)** | Mind runs on Railway 24/7 without Ford’s laptop |
| **[reality/](./reality/)** | Cold hard truth of every AO FE/BE change — cortex reads on wake; ships append |
| **[mind_sandbox/](./mind_sandbox/)** | Free-run arena vs Ford; weekly scorecard (no prod merge/deploy) |

## This directory

Job artifacts and research writeups (utility adapters, HAR plans, etc.) land here as `*.md` files from code-hire / expand work. **Do not** overwrite `HOW_NOT_TO_CRASH.md` or this README with one-off research.
